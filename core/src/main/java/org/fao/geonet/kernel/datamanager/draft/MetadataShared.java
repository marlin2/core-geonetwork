...
public class MetadataShared implements ApplicationListener<MetadataShare> {
    ...
    public void onApplicationEvent(MetadataShare event) {
        ...
        try {
            IMetadata md = manager.getMetadataObject(idmd);
            if (md instanceof MetadataDraft) {
                ...
                case ADD:
                    ...
                    operationAllowed = operationAllowedRepository
                            .findOneById_GroupIdAndId_MetadataIdAndId_OperationId(
                                    event.getOp().getId().getGroupId(),
                                    original.getId(),
                                    event.getOp().getId().getOperationId());

                    if (operationAllowed == null) {
                        operationAllowed = new OperationAllowed(
                                new OperationAllowedId()
                                        .setGroupId(event.getOp().getId()
                                                .getGroupId())
                                        .setMetadataId(original.getId())
                                        .setOperationId(event.getOp().getId()
                                                .getOperationId()));

...
public class BaseMetadataOperations implements IMetadataOperations {
    ...
    public void deleteMetadataOper(ServiceContext context, String metadataId, boolean skipAllReservedGroup) throws Exception {
        ...
        if (skipAllReservedGroup) {
            Integer[] exclude = new Integer[] { ReservedGroup.all.getId(), ReservedGroup.intranet.getId(), ReservedGroup.guest.getId() };
    ...
    private Optional<OperationAllowed> getOperationAllowedToAdd(
            final ServiceContext context, final int mdId, final int grpId,
    ...
    public List<OperationAllowed> getAllOperations(Integer mdId) {
      ...
      return opAllowedRepo.findAll(spec );
    }
